package com.nextcont.ecm.fileengine;
...
public class FileUtil {
     ...
     * @return
     */
    public static ArrayList<File> getDirFiles(String dirPath, final String prefix) {
        File path = new File(dirPath);
    ...
    public List<String> splitBySize(String fileName, int byteSize) throws IOException {
        ...
        ThreadPoolExecutor threadPool =
                new ThreadPoolExecutor(count, count * 3, 1, TimeUnit.SECONDS, new ArrayBlockingQueue<>(count * 2));
     ...
     * @throws IOException
     */
    public byte[] mergePartFiles(String partFilePrefix,int partFileSize, String mergeFileName) throws IOException {
        ...
        try {
            ArrayList<File> partFiles = FileUtil.getDirFiles("/", partFilePrefix);
            ...
            CountDownLatch latch = new CountDownLatch(partFiles.size());

            for (int i = 0; i < partFiles.size(); i++) {
                threadPool.execute(new MergeRunnable(latch,i * partFileSize, mergeFileName, partFiles.get(i)));
            }
            ...
            File mergerdFile = new File(mergeFileName);

            fileByte = IOUtils.toByteArray(new FileInputStream(mergerdFile));
    ...
    private class MergeRunnable implements Runnable {
        ...
        public MergeRunnable(CountDownLatch latch ,long startPos, String mergeFileName, File partFile) {
            this.startPos = startPos;
